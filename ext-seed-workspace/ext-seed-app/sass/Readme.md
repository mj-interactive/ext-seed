...
folder are mapped to the application's root namespace, 'ExtSeedApp'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "ExtSeedApp"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'ExtSeedApp' in the same way
as `"ExtSeedApp/sass/src"`.
